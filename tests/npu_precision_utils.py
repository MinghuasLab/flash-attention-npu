...
def compare_cv(golden: torch.Tensor, actual: torch.Tensor):
    ...
    result = (
        (mare_npu < err_threshold * 15)
        and (mere_npu < err_threshold)
        and (rmse_npu < err_threshold)
        and (abs(eb_npu) < eb_threshold)
    )
...
def compare_rule(golden: torch.Tensor, actual: torch.Tensor, ratios=(0.001, 0.001, 0.005, 0.005)):
    """
