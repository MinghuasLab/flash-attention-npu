...
#   num_heads,kv_heads in {(6,6),(6,1),(6,3)}, head_size=A, (q_seqlen,kv_seqlen)=seqA, (window_left,window_right)=winA
    ...
    # Known failure: the causal long-KV variant produces an output Inf mask
    # that does not match either reference implementation.
...
#   num_heads,kv_heads in {(6,6),(6,1),(6,3)}, head_size=B, (q_seqlen,kv_seqlen)=seqB, (window_left,window_right)=winB
    ...
    # Known failure: paged TND split-KV produces broadly incorrect output at
    # this 129/513 boundary (max|actual-ref|=9.875; 166460 elements out of tolerance).
    ...
    # Known failure: the 129/513, D=224, split-KV case produces broadly
    # incorrect output (max|actual-ref|=9.890625; 460637 elements out of tolerance).
...
hd_cases = [
    ...
    # Known failure: paged causal TND at the 128/129 boundary with D=63
    # produces broadly incorrect output (max|actual-ref|=9.9375; 613636 elements out of tolerance).
    # (torch.bfloat16, 5, 24, 4, 128, 129, 63, 1, 128, True, "TND", 1, -1, -1, 0.0),
