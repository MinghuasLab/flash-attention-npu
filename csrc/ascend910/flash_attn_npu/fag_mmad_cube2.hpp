...
struct BlockMmad<MmadAtlasA2FAGCube2, L1TileShape_, L0TileShape_, AType_, BType_, CType_, BiasType_, TileCopy_,
                 TileMmad_> {
  public:
    // Type Aliases
                    ...
                    uint32_t& pingpongFlagL0B)
    {
        ...
        for (uint32_t i = 0; i < addrs.blockLength; ++i) {
            ...
            for (uint32_t n_loop_index = 0; n_loop_index < n_loop; n_loop_index++) {
                ...
                AscendC::LocalTensor<ElementB>* l1_b_buf_tensor =
                    pingpongFlagL1B ? &l1_b_pong_tensor : &l1_b_ping_tensor;
                AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(pingpongFlagL1B);

                auto layoutTileB =
                    layoutB.GetTileLayout(MakeCoord(static_cast<uint32_t>(n_remain), static_cast<uint32_t>(headdim)));
                ...
                for (uint32_t m_loop_index = 0; m_loop_index < m_loop; m_loop_index++) {
                    AscendC::LocalTensor<ElementA>* l1_a_buf_tensor =
                    ...
                    if (!is_skip) {
                        auto layoutTileA = layoutA.GetTileLayout(
                            MakeCoord(static_cast<uint32_t>(m_remain), static_cast<uint32_t>(n_remain)));
                        LayoutAInL1 layoutAInL1 = LayoutAInL1::template MakeLayout<ElementA>(m_remain, n_remain);
                        copyGmToL1A(*l1_a_buf_tensor,
                                    gLeft[(m_loop * n_loop_index + m_loop_index - skip_num) * 128 * 128], layoutAInL1,
                                    layoutTileA);
                    }
                ...
                AscendC::LocalTensor<ElementB>* l0_b_buf_tensor =
                    pingpongFlagL0B ? &l0_b_pong_tensor : &l0_b_ping_tensor;
                ...
                for (uint32_t m_loop_index = 0; m_loop_index < m_loop; m_loop_index++) {
                    AscendC::LocalTensor<ElementA>* l1_a_buf_tensor =
                        pingpongFlagL1A ? &l1_a_pong_tensor : &l1_a_ping_tensor;
                    AscendC::LocalTensor<ElementA>* l0_a_buf_tensor =
                    ...
                    bool last_k = false;
                    last_k =
                        (m_loop_index == 0 && upperRight) ? n_loop_index == n_loop - 2 : n_loop_index == n_loop - 1;
