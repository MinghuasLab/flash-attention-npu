...
struct BlockMmad<MmadAtlasA2FAGCube1, L1TileShape_, L0TileShape_, AType_, BType_, CType_, BiasType_, TileCopy_,
                 TileMmad_> {
  public:
    // Type Aliases
    ...
    CATLASS_DEVICE
    void cube1_base_matmul(LocalTensor<ElementA>* l1_a_tensor, LocalTensor<ElementB>* l1_b_tensor,
                           GlobalTensor<ElementC>* gOut, uint32_t& pingpongFlagL1A, uint32_t& pingpongFlagL0A,
                           uint32_t& pingpongFlagL1B, uint32_t& pingpongFlagL0B, uint32_t& pingpongFlagC,
            ...
            for (int m_offset = 0; m_offset < l1_m_size_; m_offset += SIZE_128) {
                ...
                if (!l0_skip_flag) {
                    AscendC::MmadParams commonMadParams{
                        BASE_BLOCK_LENGTH, BASE_BLOCK_LENGTH, BASE_BLOCK_LENGTH, 3, false, true};
            ...
            for (int m_index = 0; m_index < m_loop; m_index++) {
                ...
                for (int n_index = 0; n_index < n_loop; n_index++) {
                    ...
                    cube1_base_matmul(l1_a_tensor, l1_b_tensor, &gOut, pingpongFlagL1A, pingpongFlagL0A,
                                      pingpongFlagL1B, pingpongFlagL0B, pingpongFlagC, l1_m_size_, l1_n_size_,
                                      upper_right_flag);
