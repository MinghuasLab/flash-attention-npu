        ...
        )
{
    ...
    if (out_.has_value()) {
        ...
        // The kernel's output contract is dense BSHD/TND even when q is a
        // strided view. Do not inherit q's preserve-format strides here.
    ...
    const QkvElementStrides kNewStrides = appendKV
        ? GetQkvElementStrides(k_new_.value(), "k_new")
        : QkvElementStrides{0, static_cast<uint64_t>(num_heads_k * head_size_og),
                            static_cast<uint64_t>(head_size_og)};
    const QkvElementStrides vNewStrides = appendKV
        ? GetQkvElementStrides(v_new_.value(), "v_new")
        : QkvElementStrides{0, static_cast<uint64_t>(num_heads_k * v.size(-1)),
