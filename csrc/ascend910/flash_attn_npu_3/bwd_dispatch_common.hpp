...
#define BWD_KERNEL_LAUNCH(DT, DTYPE, IS_MASK, IS_DTM, IS_SOFTCAP)                                                      \
    do {                                                                                                               \
        FAGGeneral<DT, DTYPE, kInputLayout, IS_MASK, 0, IS_DTM, IS_SOFTCAP><<<blockDim, nullptr, aclStream>>>(         \
            fftsAddr, dOutDevice, qDevice, kDevice, vDevice, outDevice, nullptr, attenMaskDevice, softMaxLseDevice,    \
            cuSeqQlenDevice, cuSeqKvlenDevice, dqDevice, dkDevice, dvDevice, nullptr, workspaceDevice, tilingDevice);  \
    } while (0)
    ...
    auto launch_fag_general_kernel = [=]() -> int {
        BWD_BOOL_SWITCH(is_softcap, HasSoftcap, {
            BWD_BOOL_SWITCH(has_attn_mask, IsAttenMask, {
                BWD_BOOL_SWITCH(deterministic, IsDtm, { BWD_LAUNCH_HD(DType, IsAttenMask, IsDtm, HasSoftcap); });
            });
        });
        return 0;
    };
