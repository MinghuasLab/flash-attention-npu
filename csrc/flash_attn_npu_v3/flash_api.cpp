        ...
        )
{
    ...
    if (is_bf16) {
        if (paged_KV) {
            if (is_local) {
                if (is_varlen_q) {
                    SplitFuse::FAInfer<bfloat16_t, bfloat16_t, float, true, FaiKenel::MaskType::MASK_SWA, FaiKenel::inputLayout::TND, Catlass::Epilogue::LseModeT::OUT_ONLY><<<launchBlockDim, nullptr, aclStream>>>(
        ...
        if (paged_KV) {
            if (is_local) {
                if (is_varlen_q) {
                    SplitFuse::FAInfer<half, half, float, true, FaiKenel::MaskType::MASK_SWA, FaiKenel::inputLayout::TND, Catlass::Epilogue::LseModeT::OUT_ONLY><<<launchBlockDim, nullptr, aclStream>>>(
                            fftsAddr, qDevice, kDevice, vDevice, maskDevice, blockTableDevice, oDevice, softmaxLseDevice,
                            qSeqDevice, kvSeqDevice, workspaceDevice, tilingDevice);
                } else {
            ...
            if (is_local) {
                if (is_varlen_q) {
                    SplitFuse::FAInfer<half, half, float, false, FaiKenel::MaskType::MASK_SWA, FaiKenel::inputLayout::TND, Catlass::Epilogue::LseModeT::OUT_ONLY><<<launchBlockDim, nullptr, aclStream>>>(
                            fftsAddr, qDevice, kDevice, vDevice, maskDevice, blockTableDevice, oDevice, softmaxLseDevice,
                            qSeqDevice, kvSeqDevice, workspaceDevice, tilingDevice);
                } else {
