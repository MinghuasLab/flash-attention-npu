...
class FagPost {
  public:
    CATLASS_DEVICE void Init(Catlass::Arch::Resource<ArchTag>& resource, GM_ADDR dq, GM_ADDR dk, GM_ADDR dv,
                             GM_ADDR workspace, GM_ADDR tiling)
    {
        ...
        for (uint32_t slot = 0; slot < 2U; ++slot) {
            ...
            dstUb_[slot] = resource.ubBuf.template GetBufferByByte<DataType>(slotOffset + tileElements * sizeof(float));
        }
    }
                                   ...
                                   event_t vToMte3Ping, event_t vToMte3Pong)
    {
        ...
        if (!privDkv) {
            ProcessRegion<false>(dvGm_, dvWorkspace_, dvCount, 1.0F, vectorCoreId, vectorCoreNum, pingPongIdx,
                                 mte3ToMte2Ping, mte3ToMte2Pong, mte2ToVPing, mte2ToVPong, vToMte3Ping, vToMte3Pong);
            ProcessRegion<true>(dkGm_, dkWorkspace_, dkCount, tiling_->scaleValue, vectorCoreId, vectorCoreNum,
                                pingPongIdx, mte3ToMte2Ping, mte3ToMte2Pong, mte2ToVPing, mte2ToVPong, vToMte3Ping,
                                vToMte3Pong);
        }
        ...
        if (tiling_->dqPostAbsorb == 0) {
            ProcessRegion<true>(dqGm_, dqWorkspace_, dqCount, tiling_->scaleValue, vectorCoreId, vectorCoreNum,
                                pingPongIdx, mte3ToMte2Ping, mte3ToMte2Pong, mte2ToVPing, mte2ToVPong, vToMte3Ping,
                                vToMte3Pong);
        }
  ...
  private:
    ...
    CATLASS_DEVICE void ProcessRegion(AscendC::GlobalTensor<DataType>& dst, AscendC::GlobalTensor<float>& src,
                                      uint64_t elementCount, float scale, uint32_t vectorCoreId, uint32_t vectorCoreNum,
                                      uint32_t& pingPongIdx, event_t mte3ToMte2Ping, event_t mte3ToMte2Pong,
                                      event_t mte2ToVPing, event_t mte2ToVPong, event_t vToMte3Ping,
                                      event_t vToMte3Pong)
    {
        constexpr uint32_t tileElements = 20U * 1024U;
        if (vectorCoreNum == 0U)
        ...
        while (done < rangeCount) {
            ...
            AscendC::DataCopyExtParams inputParams{1, static_cast<uint32_t>(current * sizeof(float)), 0, 0, 0};
            ...
            AscendC::Cast(dstUb_[slot], srcUb_[slot], AscendC::RoundMode::CAST_RINT, current);
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(vToMte3Event);
            AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(vToMte3Event);
            AscendC::DataCopyExtParams outputParams{1, static_cast<uint32_t>(current * sizeof(DataType)), 0, 0, 0};
