    ...
    int64_t sm_margin)
{
    ...
    TORCH_CHECK(sm_margin == 0,
                "Ascend950 v4 bwd does not support sm_margin");
    TORCH_CHECK(q.dtype() == at::kHalf || q.dtype() == at::kBFloat16,
                "Ascend950 v4 bwd only supports FP16 and BF16");
    ...
    TORCH_CHECK(q.dim() == (is_varlen ? 3 : 4),
                "Ascend950 v4 bwd: q must use TND or BSND layout");
    ...
    TORCH_CHECK(q_seqlen > 0 && kv_seqlen > 0,
                "Ascend950 v4 bwd: sequence lengths must be positive");
    ...
    TORCH_CHECK(qk_head_dim == k_head_dim,
                "Ascend950 v4 bwd: q and k must share the same head dimension");
    TORCH_CHECK(qk_head_dim > 0 && qk_head_dim <= 256,
                "Ascend950 v4 bwd: q/k head dimension must be in (0, 256]");
    TORCH_CHECK(v_head_dim > 0 && v_head_dim <= 256,
                "Ascend950 v4 bwd: v head dimension must be in (0, 256]");
    ...
    if (is_varlen) {
        ...
        TORCH_CHECK(q_lengths[0] == 0 && kv_lengths[0] == 0,
                    "Ascend950 v4 bwd: cu_seqlens must start at zero");
    ...
    TORCH_CHECK(tiling_status == 0,
                "Ascend950 v4 bwd: arch35 GetFAGTilingParam failed");
