                                ...
                                bool return_lse)
{
    ...
    fill_inference_context(ctx, scratch, q, k, v, is_varlen_q ? &cu_seqlen_q_cpu : nullptr, &seqlens_k_cpu, paged_KV,
                           page_block_size, num_blocks, max_num_blocks_per_seq, is_causal, is_local,
                           /* window_size_left= */ is_local ? window_size_left : 0,
                           /* window_size_right= */ is_local ? window_size_right : 0, is_varlen_q, is_bf16, batch_size,
                           seqlen_q, num_heads, num_heads_k, head_size_q, head_size_v,
    ...
    const uint32_t launchBlockDim = flashDecodeEnabled ? tilingData.fdActiveCoreNum : blockDim;
    ...
    TORCH_CHECK(sync_err == ACL_SUCCESS,
                "950 backend (v4): aclrtSynchronizeStream failed after launch_fwd, err=", sync_err);
