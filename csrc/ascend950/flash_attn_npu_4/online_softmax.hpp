...
template <class OutputType_, class ElementS_, class LayoutS_, class MaskType_, class TileCopy_>
class BlockEpilogue<EpilogueFAOnlineSoftmax, OutputType_, Gemm::GemmType<ElementS_, LayoutS_>, MaskType_, TileCopy_> {
  public:
        ...
        if (isFirstKvSTile) {
            if (n > 64) {
                ComputeScaleAndMax<ElementInput, ElementOutput, false>(
                    ...
                    nPadding, scaleValue, 128, blockStride, nRound, rescaleThreshold);
            } else {
            ...
            if (n > 64) {
                ComputeScaleAndMax<ElementInput, ElementOutput, true>(
                    ...
                    scaleValue, 128, blockStride, nRound, rescaleThreshold);
            } else {
        ...
        if (isFirstKvSTile) {
            if (mAligned16TileNum == 0) {
                ComputeScaleAndMaxDn<ElementInput, ElementOutput, false, MAligendTileNum::Zero>(
                    ...
                    64, blockStride, nRound, expMaxUbAddr, lastSumAddr, rescaleThreshold);
            ...
            if (mAligned16TileNum == 0) {
                ComputeScaleAndMaxDn<ElementInput, ElementOutput, true, MAligendTileNum::Zero>(
                    ...
                    blockStride, nRound, expMaxUbAddr, lastSumAddr, rescaleThreshold);
            } else if (mAligned16TileNum == 1) {
        ...
        if (isFirstKvSTile) {
            if (n > 64) {
                ComputeScaleAndMaxMask<ElementInput, ElementOutput, false>(
                    ...
                    tailN, nPadding, scaleValue, 128, blockStride, nRound, rescaleThreshold);
            } else {
            ...
            if (isFirstKvSTile) {
                if (n > 64) {
                    ComputeScaleAndMaxMaskInvert<ElementInput, ElementOutput, false>(
                        ...
                        nLoops, tailN, nPadding, scaleValue, 128, blockStride, nRound, rescaleThreshold);
                } else {
  ...
  private:
    ...
    ComputeScaleAndMax(__ubuf__ ElementS* srcUb, __ubuf__ ElementS* newMaxUb, __ubuf__ ElementS* newMaxUbStart,
                       ...
                       uint16_t S2BaseSize, uint32_t blockStride, uint32_t repeatStride, float rescaleThreshold)
    {
    ...
    template <typename ElementS, typename ElementP, bool isUpdate>
    __simd_vf__ static inline void ComputeScaleAndMaxMaskInvert(
        __ubuf__ ElementS* srcUb, __ubuf__ ElementS* newMaxUb, __ubuf__ ElementS* newMaxUbStart,
        __ubuf__ ElementS* LastMaxUbStart, __ubuf__ ElementP* expUb, __ubuf__ ElementS* expSumUb,
        __ubuf__ ElementMask* maskUb, uint16_t m, uint16_t nLoops, uint32_t tailN, uint32_t nPadding,
        ElementInput dScale, uint16_t S2BaseSize, uint32_t blockStride, uint32_t repeatStride, float rescaleThreshold)
    {
        using namespace AscendC::MicroAPI;
        RegTensor<float> minVreg;
        RegTensor<float> srcVreg;
    ...
    ComputeScaleAndMaxMask64(__ubuf__ ElementS* srcUb, __ubuf__ ElementS* newMaxUb, __ubuf__ ElementS* newMaxUbStart,
                             ...
                             uint32_t repeatStride, float rescaleThreshold)
    {
        using namespace AscendC::MicroAPI;
        RegTensor<float> minVreg;
        RegTensor<float> srcVreg;
    ...
    template <typename ElementS, typename ElementP, bool isUpdate>
    __simd_vf__ static inline void ComputeScaleAndMaxMaskInvert64(
        __ubuf__ ElementS* srcUb, __ubuf__ ElementS* newMaxUb, __ubuf__ ElementS* newMaxUbStart,
        __ubuf__ ElementS* LastMaxUbStart, __ubuf__ ElementP* expUb, __ubuf__ ElementS* expSumUb,
        __ubuf__ ElementMask* maskUb, uint16_t m, uint16_t nLoops, uint32_t tailN, uint32_t nPadding,
        ElementInput dScale, uint16_t S2BaseSize, uint32_t blockStride, uint32_t repeatStride, float rescaleThreshold)
    ...
    ComputeScaleAndMaxDn(__ubuf__ ElementS* srcUb, __ubuf__ ElementS* newMaxUb, __ubuf__ ElementS* LastMaxUbStart,
                         ...
                         __ubuf__ ElementS* lastExpSumUb, float rescaleThreshold)
    {
        using namespace AscendC::MicroAPI;
